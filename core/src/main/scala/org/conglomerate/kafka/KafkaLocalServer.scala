...
object KafkaLocalServer {
  ...
  def apply(kafkaPort: Int, zookeeperServerPort: Int, cleanOnStart: Boolean): KafkaLocalServer = {
    ...
    val kafkaProperties = createKafkaProperties(kafkaPort, zookeeperServerPort,
      kafkaDataDir)

    if (cleanOnStart)
      deleteDirectory(kafkaDataDir)
  ...
  def deleteDirectory(directory: File): Unit = {
    if (directory.exists())
      try {
        ...
        val files = Files.walk(rootPath, FileVisitOption.FOLLOW_LINKS).sorted(Comparator.reverseOrder())
          .iterator().asScala
...
private class ZooKeeperLocalServer(port: Int, cleanOnStart: Boolean) {
  ...
  def start(): Unit = {
    ...
    if (cleanOnStart)
      deleteDirectory(zookeeperDataDir)
